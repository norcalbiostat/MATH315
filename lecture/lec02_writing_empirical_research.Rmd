...
This should **not**  be an exact copy of the information found in the codebook. You must rephrase this into your own words. 
...
* Describe (at least) both the explanatory and response variables. 
* Do NOT use your variable names or "speak in code". 
* Describe any data management done - specifically collapsing levels and aggregating variables. 
...
> Attitude toward school was measured with a questionnaire developed for use in this study. It contains nine statements. The first three measure attitudes toward academic subjects; the next three measure attitudes toward teachers, counselors, and administrators; the last three measure attitudes toward the social environment in the school. Participants were asked to rate each statement on a five-point scale from 1 (strongly disagree) to 5 (strongly agree). Aggregate measures of agreement for each of the three topics were created by averaging the individual questions within each topic. 

This measures discussion is successful because it indicates how attitudes were measured (ranking on a five-point scale) and how new variables were created (averaging across variables). No variable names were used. 
...
Using appropriate graphing and summary methods, write a statement that describes what you have learned about your research topic thus far. Explain what this finding tells you about your research question. Describe what you expect to find in subsequent statistical analysis. What are you going to do with this information? 

> After exploring the relationship between smoking status and depression using side by side barcharts, I found that individuals with signs of major depression (N=, % of sample) report on average a greater number of nicotine dependence episodes (mean=, sd=) compared to those not exibiting signs (N=, % of sample, mean=, sd=). This indicates that my hypothesis that depression contributes to the severity of nicotine dependence may be upheld. Furthermore, using a contingency table I found that a greater number of females report depressive symptoms (%=, number depressed/number females) compared to males (%=, number depressed/number males). I need to check if there is a difference in smoking status between males and females as well, this could be an important factor for my research. 
